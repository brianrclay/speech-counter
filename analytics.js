...
(() => {
    ...
    const events = new Set(['board_add', 'board_remove', 'board_clear', 'board_count', 'board_undo',
        ...
        'session_save', 'sync_complete', 'sync_error', 'session_edit', 'session_delete', 'export_data', 'paywall_view', 'select_plan',
    ...
    function syncContext(explicitPreference = false) {
        const run = async () => {
            ...
            if (enabled()) {
                ...
                if (!enabled()) body = { enabled: false };
                else if (!/^\d+\.\d+$/.test(clientId || '')) return;
                else body = { enabled: true, clientId, sessionId: String(sessionId || '') };
            }
            body.explicitPreference = explicitPreference;
            if (window.SpeechStore.session.get()?.token !== session.token) return;
            try {
                await account.call('/api/analytics-context', body, session.token, { signal: AbortSignal.timeout(1500) });
                return true;
    ...
    window.SpeechAnalytics = { track, enabled, setEnabled, syncContext };
    init();
