...
const analytics = { enabled: true, consentVersion: 'purchase-v1', experimentId: 'web_default_plan_v1', variantId: 'monthly', clientId: '123.456', sessionId: String(Math.floor(now / 1000)), updatedAt: now };
...
test('honors missing consent and does not send PII or stale session attribution', () => {
    ...
    assert.equal(payloadFor(revenue, { ...analytics, consentVersion: undefined }, now), null);
    assert.equal(payload.events[0].params.variant_id, 'monthly');
    assert.equal(payload.events[0].params.experiment_id, 'web_default_plan_v1');
