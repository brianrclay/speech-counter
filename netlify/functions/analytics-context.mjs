...
export default endpoint(async (req, context) => {
    ...
    if (body.enabled && body.consentVersion !== 'purchase-v1') throw fail(400, 'Purchase analytics consent required');
    if (body.experimentId !== undefined && (body.experimentId !== 'web_default_plan_v1' || !['monthly', 'lifetime'].includes(body.variantId))) throw fail(400, 'Invalid experiment');
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        auth.account.analytics = body.enabled
            ? { enabled: true, clientId: body.clientId, sessionId: body.sessionId || null, updatedAt: Date.now(), consentVersion: 'purchase-v1', consentedAt: auth.account.analytics?.consentedAt || Date.now(),
                experimentId: auth.account.analytics?.experimentId || body.experimentId || null,
                variantId: auth.account.analytics?.variantId || body.variantId || null }
